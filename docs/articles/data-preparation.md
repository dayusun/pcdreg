...
Getting the data into the right shape is most of the work of fitting
these models, and it is where the mistakes are. This vignette covers
what the two fitters expect, how to say it in a formula, how to convert
from the layouts data usually arrives in, and what each error message
means.

## What panel count data is

A subject is examined at times $T_{i1} < \ldots < T_{iJ_{i}}$. At each
examination you learn how many events have happened since the previous
one, but not when any of them happened. Two quantities matter and are
easy to confuse:
...
Interval starts are filled in from each subject’s previous examination,
with follow-up beginning at zero, which is what the printed intervals
show. Subject 1 contributes $(0,0.5\rbrack$, $(0.5,1.1\rbrack$ and
...
confounded there, and the information matrix is singular. Real fits
appear later in this vignette.

### Time-varying covariates: counting process rows

When a covariate changes between examinations, you need a row for each
stretch over which the covariates are constant. This is exactly the
layout `coxph()` uses for time-dependent covariates.
...
Read it as: subject 1 was examined at 0.5 with 2 events and at 1.2 with
none, and their dose changed from 10 to 25 at time 0.9, part way through
the second examination interval.
...
Use the four argument form as soon as one covariate moves. If in doubt
use the four argument form: it can express anything the three argument
form can.

## The formula

The right hand side is an ordinary model formula, so the usual things
work.
...
The dot expands to every column that is not part of the response, so
...
One consequence worth knowing: adding a constant $c$ to a covariate
leaves $\beta$ untouched and rescales the baseline by $e^{- c\beta}$.
Centring your covariates changes the baseline, not the coefficients.

### Row order does not matter

The data are sorted internally by subject and time, so you do not need
to `arrange()` first. Nor do subject identifiers need to be integers or
consecutive: numbers, characters and factors all work, and the original
labels come back in `predict(type = "mean")`.

## Recipes for common layouts

### Cumulative counts

The most common and most damaging mismatch. If your `count` column
accumulates, difference it within subject before fitting.
...
Then fit with `increment`. With `NA`s present for covariate-change rows,
do the differencing on the examination rows alone:
...
That layout repeated each panel count on every row of its interval and
marked examinations with a separate indicator. A repeated count carries
nothing the count on the examination row does not, so the conversion is
one line:
...
If visits and covariate changes live in different tables, stack them and
carry the covariates forward. The examination rows keep their counts;
the covariate change rows get `NA`.
...
Wide data needs reshaping to long first;
[`stats::reshape()`](https://rdrr.io/r/stats/reshape.html) or
[`tidyr::pivot_longer()`](https://tidyr.tidyverse.org/reference/pivot_longer.html)
both do it. Aim for one row per subject and visit, then use the three
argument form.

## What the data must satisfy

Both fitters check these before doing any arithmetic, and name the
...
If your times are calendar dates, or follow-up starts at enrolment,
subtract each subject’s own origin first.
...
follow-up. Carry the last value forward rather than leaving the stretch
out.

**Every subject has at least one examination.** A subject whose `count`
is `NA` on every row contributes nothing and is more likely a data error
than an intention.
...
#> Error: `count` must be a non-negative whole number at examination times.
```

**One row per subject may end at any given time.** Duplicate rows for
the same subject and time are rejected rather than silently combined.

### What gets dropped

Follow-up ends at a subject’s last examination: the likelihood runs over
$j = 1,\ldots,J_{i}$ and knows nothing beyond $T_{iJ_{i}}$. Rows lying
entirely after it are dropped, with a message saying how many.
...
If that message surprises you, the usual cause is a censoring row
appended after the last visit. It is harmless: no information is lost,
because there was none in that stretch to begin with.
...
The pooled grid $t_{1} < \ldots < t_{K}$ is the set of all distinct
examination times across all subjects. For the **rate model**, each
subject’s covariate trajectory is evaluated at every grid time inside
their follow-up, which is why the whole trajectory must be present and
contiguous. For the **means model** only the subject’s own examination
...
Ties across subjects shrink the grid, and the grid drives the cost of
the rate model. Examination times recorded to the day or the week tie
naturally; times carried at full numerical precision never do, and the
grid then grows with the sample size. Rounding to the precision the
visits were actually scheduled at is worth doing.

## Checking before you fit

The quantities above are the quickest sanity check: `nexam` should match
the number of examinations you think you have, and `nevent` the total
number of events. If `nevent` is far larger than expected, the usual
explanation is cumulative counts.
