...
fit <- panelrate(PanelCount(id, tstart, tstop, count) ~ x1 + x2, data = dat)
...
test_that("factors and interactions are handled and no intercept is fitted", {
  ...
  f <- panelrate(PanelCount(id, tstart, tstop, count) ~ x1 + g, data = d)
  expect_false("(Intercept)" %in% names(coef(f)))
  expect_named(coef(f), c("x1", "godd"))

  fi <- panelrate(PanelCount(id, tstart, tstop, count) ~ x1 * x2, data = d)
  expect_named(coef(fi), c("x1", "x2", "x1:x2"))
})

test_that("subset is honoured", {
  half <- panelrate(PanelCount(id, tstart, tstop, count) ~ x1 + x2, data = dat,
                    subset = id <= 20)
  expect_equal(nobs(half), 20L)
})

test_that("a non PanelCount response is refused", {
  expect_error(panelrate(count ~ x1, data = dat), "PanelCount")
})
